...
const data = {
  ...
  sellAmount: process.env.SELL_AMOUNT_OF_COIN, // how much you want to sell in Coin
  ...
  Slippage: process.env.SELL_SLIPPAGE, //in Percentage

  gasPrice: process.env.SELL_GWEI, //in gwei

  gasLimit: process.env.SELL_GAS_LIMIT, //at least 21000
...
let sellAction = async () => {
  ...
  try {
    ...
    const amountIn = ethers.utils.parseUnits(`${data.sellAmount}`, "ether");
    ...
    console.log(
      chalk.green.inverse(`Start to buy \n`) +
        `Selling Token
        =================
        tokenIn: ${amountIn.toString()} ${tokenIn} (COIN)
        tokenOut: ${amountOutMin.toString()} ${tokenOut} (WBNB)
      `
    );
    ...
    console.log(`Error caused by : 
        ...
        message : Please check your BNB/WBNB/Coin balance or approve your token manually on Pancakeswap
        }`);
    console.log(error);
